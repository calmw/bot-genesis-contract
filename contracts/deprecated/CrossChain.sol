...
contract CrossChain is System, ICrossChain, IParamSubscriber {
    ...
    mapping(address => mapping(uint8 => bool))
        public registeredContractChannelMap;
    ...
    event receivedPackage(
        uint8 packageType,
        uint64 indexed packageSequence,
        uint8 indexed channelId
    ); // @dev deprecated
    event unsupportedPackage(
        uint64 indexed packageSequence,
        uint8 indexed channelId,
        bytes payload
    ); // @dev deprecated
    event unexpectedRevertInPackageHandler(
        address indexed contractAddr,
        string reason
    ); // @dev deprecated
    event unexpectedFailureAssertionInPackageHandler(
        address indexed contractAddr,
        bytes lowLevelData
    ); // @dev deprecated
    ...
    modifier blockSynced(uint64 _height) {
        require(
            ILightClient(LIGHT_CLIENT_ADDR).isHeaderSynced(_height),
    ...
    modifier channelSupported(uint8 _channelID) {
        require(
            channelHandlerContractMap[_channelID] != address(0x0),
    ...
    modifier onlyCabinet() {
        uint256 indexPlus = IBSCValidatorSetV2(VALIDATOR_CONTRACT_ADDR)
            .currentValidatorSetMap(msg.sender);
        uint256 numOfCabinets = IBSCValidatorSetV2(VALIDATOR_CONTRACT_ADDR)
            .numOfCabinets();
        if (numOfCabinets == 0) {
            numOfCabinets = 21;
        }
    ...
    ) external override onlyGov whenNotSuspended {
        revert("deprecated");
    }
