    ...
    ERC20Upgradeable
{
    ...
    mapping(address => DoubleEndedQueueUpgradeable.Bytes32Deque)
        private _unbondRequestsQueue;
    ...
    receive() external payable onlyStakeHub {
        uint256 index = block.timestamp /
            IStakeHub(STAKE_HUB_ADDR).BREATHE_BLOCK_INTERVAL();
    ...
    function undelegate(
        address delegator,
        uint256 shares
    ) external onlyStakeHub returns (uint256 bnbAmount) {
        if (shares == 0) revert ZeroShares();
        ...
        uint256 unlockTime = block.timestamp +
            IStakeHub(STAKE_HUB_ADDR).unbondPeriod();
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(delegator, _useSequence(delegator))
        );
    ...
    function distributeReward(
        ...
        uint256 _commission = (bnbAmount * uint256(commissionRate)) /
    ...
    function slash(
        ...
        slashShares = slashShares > selfDelegation
            ? selfDelegation
            : slashShares;
    ...
    function _useSequence(
        ...
        CountersUpgradeable.Counter storage sequence = _unbondSequence[
            delegator
        ];
