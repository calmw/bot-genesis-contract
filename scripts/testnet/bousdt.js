...
async function main() {
    ...
    const contractObj = await contract.deploy('BOHR Wrapped Tether USD', 'BOUSDT', BigNumber.from(6));
    // 等待部署完成
    ...
    await (await contractObj.mint(
        ...
        ethers.utils.parseUnits("100000000000000000000000", 6)
    )).wait();
